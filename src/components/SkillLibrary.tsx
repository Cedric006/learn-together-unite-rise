  ...
  const categories = [
    {
      ...
      color: "bg-soft-blue-100 text-royal-blue-800",
      skills: [
    ...
    },
    {
      ...
      color: "bg-royal-blue-100 text-royal-blue-800",
      skills: [
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-3xl font-bold text-blue-gray-900 mb-4">Skill Library</h2>
        <p className="text-lg text-blue-gray-600 max-w-2xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center">
        <div className="flex-1 max-w-md">
          <Input
            ...
            className="w-full bg-white border border-soft-blue-300 text-blue-gray-800"
          />
        </div>
        <div className="flex gap-2 flex-wrap">
          <Button
            ...
            className={selectedCategory === "all" ? "bg-royal-blue-600 text-white" : "bg-soft-blue-100 text-royal-blue-700 border-royal-blue-300 hover:bg-royal-blue-600 hover:text-white"}
          >
            All Categories
          </Button>
          {categories.map((category) => (
            <Button
              ...
              className={selectedCategory === category.id ? "bg-royal-blue-600 text-white" : "bg-soft-blue-100 text-royal-blue-700 border-royal-blue-300 hover:bg-royal-blue-600 hover:text-white"}
            >
              {category.name}
            </Button>
          ))}
        ...
        {filteredCategories.map((category) => (
          <div key={category.id} className="space-y-4">
            <div className="flex items-center gap-3">
              <h3 className="text-2xl font-semibold text-blue-gray-900">{category.name}</h3>
              <Badge className={category.color}>{category.skills.length} skills</Badge>
            </div>
            
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {category.skills.map((skill, index) => (
                <Card key={index} className="hover-lift cursor-pointer bg-white border border-soft-blue-200 shadow-sm">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-lg text-blue-gray-800">{skill.name}</CardTitle>
                    <CardDescription className="text-blue-gray-600">{skill.level}</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-royal-blue-600 font-medium">
                          {skill.rooms} active rooms
                        </span>
                      </div>
                      <Button className="w-full bg-royal-blue-600 text-white hover:bg-royal-blue-700" size="sm">
                        Join Community
                      </Button>
                    </div>
                  </CardContent>
      ...
      {filteredCategories.length === 0 && (
        <div className="text-center py-12">
          <p className="text-blue-gray-600 text-lg">No skills found matching your search.</p>
          <Button variant="outline" className="mt-4 bg-soft-blue-100 text-royal-blue-700 border-royal-blue-300 hover:bg-royal-blue-600 hover:text-white" onClick={() => {setSearchTerm(""); setSelectedCategory("all");}}>
            Clear Filters
          </Button>
        </div>
      )}
